...
def get_arguments():
    ...
    parser.add_argument('--sam_weight', type=str, default='/data/checkpoint/sam_vit_h_4b8939.pth',
                        help="the sam model weight path")
    ...
    parser.add_argument('--cth_maskIou', type=float, default=0.9, help="the parameter of complex images mask iou (0.9)")

    # simple images settings
    ...
    parser.add_argument('--sth_maskIou', type=float, default=0.7, help="the parameter of simple images mask iou")
...
# Calculate the saliency boundary
...
# Obtain the corresponding Box based on the initial mask
def get_propmt_by_mask(mask, min_area=300, off=20):
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # index that meet the conditions
    nice_indexs = []
    W, H = mask.shape[0], mask.shape[1]
    left, top = W, H
    right = bottom = -1

    # select the larger area
    ...
    # judge whether the bounding box is out of bounds.
